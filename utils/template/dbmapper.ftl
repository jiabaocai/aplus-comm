...
<mapper namespace="${packageName}.${moduleName}.mapper.${ClassName}Mapper">
  ...
  <sql id="Base_Column_List">
        <#list list as column>${column.typeName}<#if column_index+1 != listSize>
          ,<#else> </#if></#list>
  </sql>
    ...
    <trim prefix="where" prefixOverrides="and|or">
        <#list list as column>
            ...
            <if test="${column.columnName} !=null">
              and ${column.typeName} = #${leftBraces}${column.columnName}
              ,jdbcType=${column.jdbcType}${rightBraces}
            </if>
            <#else >
  ...
  <insert id="save" parameterType="${packageName}.${moduleName}.domain.${ClassName}">
    ...
    ,<#else></#if></#if></#list>
    )values(<#list list as column><#if column.columnName == "id"><#else >
    #${leftBraces}${column.columnName}
    ,jdbcType=${column.jdbcType}${rightBraces}<#if column_index+1 != listSize>
      ,<#else></#if></#if ></#list>)
  </insert>


  <insert id="saveRecord" parameterType="${packageName}.${moduleName}.domain.${ClassName}"
          keyProperty="id">
